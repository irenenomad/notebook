...
function ChildReconciler(shouldTrackSideEffects) {
  ...
  function reconcileSingleElement(
    ...
    while (child !== null) {
      ...
      // key相等的情况并且element.type === REACT_FRAGMENT_TYPE就是节点可以复用，并且删除其他已存在的兄弟节点
      if (child.key === key) {
        if (
          child.tag === Fragment
        ...
        // 如果没有可复用的节点，就把已有的子节点删掉
        deleteChild(returnFiber, child);
      }
      // 进入下一个兄弟节点再循环再判断
      child = child.sibling;
    }
    // 创建新的节点，调用不同的创建fiber的方法来创建
  ...
  function reconcileChildFibers(
    ...
    // 如果是<>{[]}</>那么newChild.type就会是REACT_FRAGMENT_TYPE，Fragment一定是有内容的，直接渲染子节点就可以了
    ...
    // 如果上面的条件都不满足，就有可能是null，把现有的所有children全部删掉
