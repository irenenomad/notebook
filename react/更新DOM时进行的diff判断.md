...
updateHostComponent = function(
    ...
    // 说明对象根本就没有变过，没有任何更新操作在这里发生就直接return
    ...
    // 这里根据是否有updatePayload来标记update的sideEffect，在后续commit的时候就会进行操作
    if (updatePayload) {
...
export function diffProperties(
  ...
  switch (tag) {
    case 'input':
      ...
      // 就算是没有什么更新的内容返回空数组也会被markupdate
      updatePayload = [];
      break;
    case 'option':
  ...
  // lastProps上一次循环产生的props，nextProps这一次新产生的props，这里是循环老的props
  for (propKey in lastProps) {
    if (// 如果不符合这个判断那么就是删除
      nextProps.hasOwnProperty(propKey) || // 如果新的有
  ...
  for (propKey in nextProps) {
    ...
    // 判断style是否有变化，即对应每一个style的key是否有变化
    if (propKey === STYLE) {
      if (__DEV__) {
        if (nextProp) {
      ...
      if (lastProp) {
        ...
        // 判断对应每一个style的key是否有变化，所以这里有两个循环来进行判断
      ...
      // 都没找到就是新增的，直接加入updatequene
      (updatePayload = updatePayload || []).push(propKey, nextProp);
    }
  }
  if (styleUpdates) {
    // 把styleUpdates对象做为STYLE的属性push进updatequene
    (updatePayload = updatePayload || []).push(STYLE, styleUpdates);
  }
  // 最终形成一个数组key、value两个两个为基础，后面更新的时候就会拿数组进行操作
