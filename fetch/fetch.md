##### 1、fetch优点
    （1）接口更简单、简洁，更加语义化；
    （2）基于promise，更加好的流程化控制，可以不断then把参数传递，外加 async/await，异步变同步的代码书写风格；
    （3）利于同构，isomorphic-fetch 是对 whatwg-fetch和node-fetch的一种封装，你一份代码就可以在两种环境下跑起来了；
    （4）新的web api很多内置支持fetch，比如 service worker；
##### 2、fetch缺点
    （1）兼容性；
    （2）不支持progress事件（可以借助 response.body.getRender方法来实现）；
    （3）默认不带cookie；
    （4）某些错误的http状态下如400、500等不会reject，相反它会被resolve；
    （5）不支持timeout处理；
